...
// *** These are hopelessly broken on linux, revert to the standard method ****
//----------------------------------------------------------------------------
#include <stdio.h>
#include <string.h>
#include "allegttf.h"
...
FONT *load_ttf_font_ex2_mem(AL_CONST char *filename, AL_CONST int pw, AL_CONST int ph, AL_CONST int begin,
							AL_CONST int end, AL_CONST int smooth)
{
	FONT *ret = NULL;
	char buf[L_tmpnam + 6];
	char sysbuf[512];
	...
	sprintf(sysbuf, system_string, ph, pw, smoothchar, begin, end, filename, buf);
	system(sysbuf);
...
//----------------------------------------------------------------------------
/* Load a ttf font from a file */
FONT *load_ttf_font_ex_mem(AL_CONST char *filename, AL_CONST int pw, AL_CONST int ph, AL_CONST int smooth)
{
...
//----------------------------------------------------------------------------
/* Load a ttf font from a file */
FONT *load_ttf_font_mem(AL_CONST char *filename, AL_CONST int points, AL_CONST int smooth)
{
